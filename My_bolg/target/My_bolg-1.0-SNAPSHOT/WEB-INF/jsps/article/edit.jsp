...
<head>
    ...
    <script src="${pageContext.request.contextPath}/static/js/article/edit.js"></script>
...
<body>
    <form class="layui-form" lay-filter="EditArt" id="EditArt">
        <div class="layui-form-item">
            <label class="layui-form-label">文章标题</label>
            <div class="layui-input-block">
                <input type="hidden" name="id" value="${article.id}">
                <input class="layui-input" lay-verify="title" type="text" name="title" value="${article.title}">
            </div>
        </div>
            ...
            <div class="layui-input-block">
                <select name="author_id" autocomplete="off" lay-filter="author" lay-verify="author">
                    ...
                    <c:forEach items="${users}" var="user">
                        <option value="${user.id}" <c:if test="${user.id==article.user.id}">selected</c:if>>${article.user.username}</option>
                    </c:forEach>
                </select>
            </div>
        </div>
            ...
            <div class="layui-input-block">
                <select name="menu_id" autocomplete="off" lay-filter="menu" lay-verify="menu">
                    ...
                    <c:forEach items="${m}" var="menu">
                        <option value="${menu.id}" <c:if test="${menu.id==article.menu.id}">selected</c:if>>${menu.name}</option>
                    </c:forEach>
                </select>
            </div>
        </div>
        ...
        <div class="layui-form-item layui-form-text">
            ...
            <div class="layui-input-block">
                <!-- 加载编辑器的容器 -->
                <textarea id="container" placeholder="请输入内容" class="contents" name="content" lay-verify="content">
                    ${article.content}
                </textarea>
            </div>
        </div>
        ...
        <div class="layui-form-item">
            <div class="layui-input-block">
                <button type="submit" class="layui-btn" lay-submit="" lay-filter="editSubmit">修改文章</button>
